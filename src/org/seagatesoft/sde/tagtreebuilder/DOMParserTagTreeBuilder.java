...
public class DOMParserTagTreeBuilder implements TagTreeBuilder
{
	...
	public TagTree buildTagTree(String htmlDocument, boolean ignoreFormattingTags)  throws IOException, SAXException
	{
		...
		return buildTagTree(new InputSource(instr), ignoreFormattingTags, htmlDocument);
	}
	 ...
	 */
	public TagTree buildTagTree(InputSource inputSource, String htmlDocument) throws IOException, SAXException
	{
		return buildTagTree(inputSource, false, htmlDocument);
	}
	...
	public TagTree buildTagTree(InputSource inputSource, boolean ignoreFormattingTags, String htmlDocument) throws IOException, SAXException 
	{
		...
		baseURI = htmlDocument;//documentNode.getBaseURI();
					...
					if ( tagCode == HTMLElements.IMG )
					{
						...
						if ( ! absoluteURIMatcher.matches() )
						{
							...
							//imgURI = baseURI + imgURI;
                                                    try {
                                                        imgURI = new URL(new URL(baseURI), imgURI).toString();
                                                    } catch (MalformedURLException ex) {
                                                        Logger.getLogger(DOMParserTagTreeBuilder.class.getName()).log(Level.SEVERE, null, ex);
                                                    }
						}
						...
						if ( attributesMap.getNamedItem("href") != null)
						{
							...
							if ( ! absoluteURIMatcher.matches() )
							{
								...
								//linkURI = baseURI + linkURI;
                                                            try {
                                                                linkURI = new URL(new URL(baseURI), linkURI).toString();
                                                            } catch (MalformedURLException ex) {
                                                                Logger.getLogger(DOMParserTagTreeBuilder.class.getName()).log(Level.SEVERE, null, ex);
                                                            }
							}
